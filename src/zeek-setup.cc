...
zeek::plugin::Manager* zeek::plugin::plugin_mgr = nullptr;
zeek::plugin::Manager*& plugin_mgr = zeek::plugin::plugin_mgr;
...
bool show_plugins(int level)
	{
	zeek::plugin::Manager::plugin_list plugins = zeek::plugin::plugin_mgr->ActivePlugins();
	...
	zeek::plugin::Manager::inactive_plugin_list inactives = zeek::plugin::plugin_mgr->InactivePlugins();
...
void done_with_network()
	...
	if ( net_done )
		...
		zeek::event_mgr.Dispatch(new zeek::Event(net_done,
...
void terminate_bro()
	...
	zeek::plugin::plugin_mgr->FinishPlugins();

	delete zeekygen_mgr;
	delete zeek::analyzer::analyzer_mgr;
	delete file_mgr;
...
zeek::detail::SetupResult zeek::detail::setup(int argc, char** argv,
	...
	zeek::plugin::plugin_mgr = new zeek::plugin::Manager();
	...
	zeek::plugin::plugin_mgr->SearchDynamicPlugins(bro_plugin_path());
	...
	zeek::analyzer::analyzer_mgr = new analyzer::Manager();
	...
	zeek::plugin::plugin_mgr->InitPreScript();
	zeek::analyzer::analyzer_mgr->InitPreScript();
	...
	for ( set<string>::const_iterator i = requested_plugins.begin();
		...
		{
		if ( ! zeek::plugin::plugin_mgr->ActivateDynamicPlugin(*i) )
			missing_plugin = true;
		}
	...
	zeek::plugin::plugin_mgr->ActivateDynamicPlugins(! options.bare_mode);
	...
	zeek::plugin::plugin_mgr->InitBifs();
	...
	zeek::analyzer::analyzer_mgr->InitPostScript();
